...
Follow these 9 steps in order. Use `bash` for all git operations.
...
8. **3 Wins** — Specific accomplishments anchored in the data. Not generic praise. Reference actual commit messages, LOC counts, or project names.

9. **3 Improvements** — Candid, specific suggestions based on the data. If test ratio is low, say so. If sessions are all micro, suggest deeper focus blocks. If streak broke, note when.

10. **3 Habits** — Actionable habits for the next window. Tied to the improvements.
